        ...
        >
          ...
          <template slot-scope="{data}">
            <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
              ...
              <vs-td :data="data[indextr].valor">{{data[indextr].valor | formatValue}}</vs-td>
              ...
              <vs-td :data="data[indextr].categorias">{{data[indextr].categorias | formatCategorias}}</vs-td>
              <vs-td :data="data[indextr].id" style="min-width: 100px;">
                <router-link
                  :to="{ name: 'products.edit', params: {id: data[indextr].id } }"
                >
                  <vs-button 
                    color="primary" 
                    type="filled"
  ...
  methods: {
    ...
    deleteItem(id, descricao) {
      this.$vs.dialog({
          ...
          text: `Você tem certeza que deseja apagar o produto "${id} - ${descricao}"?`,
          acceptText: 'Aceitar',
          cancelText: 'Cancelar',
          accept: () => {
